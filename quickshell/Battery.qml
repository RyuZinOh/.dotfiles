        ...
        delegate: Row {
            ...
            readonly property string batIconPath: {
                if (modelData.state === UPowerDevice.FullyCharged || modelData.state === "fully-charged") {
                    return "./svg_qml/full.qml"; // full
                }
                if (modelData.state === UPowerDevice.Charging || modelData.state === "charging") {
                    return "./svg_qml/charging.qml"; // charging
                }
                if (percentage >= 1.00) {
                    return "./svg_qml/full.qml"; // full
                }
                if (percentage >= 0.80) {
                    return "./svg_qml/80_across.qml"; // over 80
                }
                if (percentage >= 0.65) {
                    return "./svg_qml/65_accross.qml"; // over 65
                }
                if (percentage >= 0.45) {
                    return "./svg_qml/45_accross.qml"; // over 45
                }
                if (percentage >= 0.30) {
                    return "./svg_qml/30_approx.qml;"; // 30 approx
                }
                if (percentage >= 0.20) {
                    return "./svg_qml/20_approx.qml"; // 20 approx
                }

                return "./svg_qml/below_10.qml"; // below ten
            }
